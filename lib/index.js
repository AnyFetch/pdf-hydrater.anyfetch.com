...
module.exports = function(path, document, changes, finalCb) {
  async.waterfall([
    function(cb) {
      ...
      shellExec('pdf2htmlEX ' + options.join(' ') + ' --dest-dir /tmp ' + path + ' ' + outFile + ' --hdpi ' + config.quality + ' --vdpi ' + config.quality, {timeout: 45000, killSignal: 'SIGKILL'}, function(err, stdout, stderr) {
